...
var intArray = [9, 9, 4, 5];
function maxProduct(input) {
    var max = 1;
    var length = input.length;
    for (var j = 0; j < length - 1; j++) {
        var output = input[j] * input[j + 1];
        if (output > max) {
            max = output;
        }
    }
...
// 0 39    
var seqArray = [34, 1, 12, 0, 45, 54, 65]; //[45, 40, 0, 39, 10];
...
// }
function longAscSequence(input) {
    var max = 1;
    var currentLength = 1;
    var longestSeqIndex = 0;
    var currentIndex = 0;
    ...
    console.log("The max value is ".concat(max));
    for (var i = longestSeqIndex; i < longestSeqIndex + max; i++) {
        console.log(input[i]);
    }
    ;
}
